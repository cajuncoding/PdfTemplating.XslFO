...
{
    public abstract class BaseRazorViewPdfRenderer<TViewModel>: IPdfRenderer<TViewModel>
    {

        #region Abstract Constructor that all implementations must provide
        public BaseRazorViewPdfRenderer(string razorViewPath, ControllerContext controllerContext)
        {
        ...
        /// <returns></returns>
        public virtual byte[] RenderPdf(TViewModel viewModel)
        {
            //Render the XSL-FO output from the Razor Template and the View Model
            var xslFODoc = this.RenderXslFOXml(viewModel);
        ...
        protected virtual byte[] RenderXslFOPdfBytes(XDocument xslFODoc, XslFOPdfOptions xslFOPdfOptions)
        {
            ...
            //***********************************************************
            var xslFOPdfRenderer = new XslFOPdfRenderer();
            var pdfBytes = xslFOPdfRenderer.RenderPdfBytes(xslFODoc, xslFOPdfOptions);
            return pdfBytes;
        }
