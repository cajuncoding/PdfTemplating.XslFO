...
{
    public abstract class BaseXsltPdfRenderer<TViewModel>: IPdfRenderer<TViewModel>
    {

        #region Abstract Constructor that all implementations must provide
        public BaseXsltPdfRenderer(string xsltPath)
        {
            this.XsltPath = xsltPath;
            this.XsltFileInfo = new FileInfo(this.XsltPath.StartsWith("~")
                                            ? HttpContext.Current.Server.MapPath(this.XsltPath)
                                            : this.XsltPath);
        }
